...
<!-- <label for="comment_whisper"><?php echo $options['whisper_form_label'] ?></label> -->
		...
		public function comment_text($text) {
			...
			if($is_whisper && (1 == $options['whisper_on'])) {
				...
				$comment_text = $msg_denied;
 				$is_Admin = appthemes_check_user_role( 'administrator' ) ; // default is current user
				...
				//} else if (1 == $viewer_id OR current_user_can( 'manage_options' )) {
                                // by Sofia: added OR current_user ... so that all admin can view whispers
